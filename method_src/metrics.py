...
def get_gauc(y_pred, y_true, campaign_id):
    y_true = np.asarray(y_true, dtype=float)
    y_pred = np.asarray(y_pred, dtype=float)
    campaign_id = np.asarray(campaign_id)

    unique_campaign_id = np.unique(campaign_id)
    aucs = []
    for cid in unique_campaign_id:
        mask = (campaign_id == cid)
        if mask.sum() < 2: 
            continue
        g_auc = get_auc(y_true[mask], y_pred[mask])
        if not np.isnan(g_auc):
            aucs.append(g_auc)

    gauc = float(np.mean(aucs)) if aucs else np.nan
...
def get_mape(y_pred, y_true):
    ...
    """
    return np.mean(np.abs(y_pred - y_true) / np.where(np.abs(y_true) < 1e-8, 1e-8, y_true))

def get_pcoc(y_pred, y_true):
    """
    PCOC = sum(y_pred) / sum(y_true)
    """
...
def get_log_mae(y_pred, y_true):
    '''
    LogMAE = mean(|log1p(y_pred) - log1p(y_true)|)
    '''
